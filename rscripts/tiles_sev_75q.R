...
ceu_s <- ggplot(uncer_raw[REG == "CEU" & VAR == "s" & RANK_SEV >= 187 & YR %in% years_ceu_s,]) +
  geom_tile(aes(x = PAR, y = MET, fill = cut(RANK_SEV, breaks = c(187, 224, 247, 250))), colour = "white") +
  scale_x_continuous(breaks = c(1,5,10)) +
  scale_y_continuous(breaks = c(1,5,10)) +
  facet_wrap(~YR, nrow = 3) +
  #scale_fill_viridis("RANK",option = "D",direction = 1, discrete = T) +
...
med_s <- ggplot(uncer_raw[REG == "MED" & VAR == "s" & RANK_SEV >= 187 & YR %in% years_med_s,]) +
  geom_tile(aes(x = PAR, y = MET, fill = cut(RANK_SEV, breaks = c(187, 224, 247, 250))), colour = "white") +
  scale_x_continuous(breaks = c(1,5,10)) +
  scale_y_continuous(breaks = c(1,5,10)) +
  facet_wrap(~YR, nrow = 3) +
...
ceu_q <- ggplot(uncer_raw[REG == "CEU" & VAR == "q" & RANK_SEV >= 187 & YR %in% years_ceu_q,]) +
  geom_tile(aes(x = PAR, y = MET, fill = cut(RANK_SEV, breaks = c(187, 224, 247, 250))), colour = "white") +
  scale_x_continuous(breaks = c(1,5,10)) +
  scale_y_continuous(breaks = c(1,5,10)) +
  facet_wrap(~YR, nrow = 3) +
...
med_q <- ggplot(uncer_raw[REG == "MED" & VAR == "q" & RANK_SEV >= 187 & YR %in% years_med_q,]) +
  geom_tile(aes(x = PAR, y = MET, fill = cut(RANK_SEV, breaks = c(187, 224, 247, 250))), colour = "white") +
  scale_x_continuous(breaks = c(1,5,10)) +
  scale_y_continuous(breaks = c(1,5,10)) +
  facet_wrap(~YR, nrow = 3) +
