...
uncer_raw <- data.table(readRDS("../../../R/Projects/PaleoHydro/extremity_ens_1_EUR_new.rds")) 
...
uncer <- uncer_raw 
colnames(uncer) <- c("reg", "var", "yr", "severity", "area", "met", "par")
...
uncer_met_range_rel = uncer_met[var!='p', .(sev_range = diff(range(severity))
  /mean(severity), area_range = diff(range(area))/mean(area)), by = .(reg, var, yr)]
...
uncer_par = uncer[, .(severity = mean(severity, na.rm = TRUE), area = mean(area, na.rm = TRUE)), by = .(reg, par, var, yr)]
uncer_par_range = uncer_par[var!='p', .(sev_range = diff(range(severity)), area_range = diff(range(area))), by = .(reg, var, yr)]
...
uncer_met_range[reg == "EUR" & var == "s", .(mean(sev_range), mean(area_range))]/uncer_par_range[reg == "EUR" & var == "s", .(mean(sev_range), mean(area_range))]
uncer_met_range[reg == "EUR" & var == "s", .(range(sev_range), range(area_range))]/uncer_par_range[reg == "EUR" & var == "s", .(range(sev_range), range(area_range))]
...
par_vs_met_area = merge(par_vs_met_area, uncer[is.na(aft_1900), mean(range_area_par), .(var, reg)], by = c("var", "reg")) 
par_vs_met_area = merge(par_vs_met_area, uncer[is.na(aft_1900), mean(range_area_met), .(var, reg)], by = c("var", "reg")) 
...
par_vs_met_sev = merge(par_vs_met_sev, uncer[is.na(aft_1900), mean(range_sev_par, na.rm = T), .(var, reg)], by = c("var", "reg")) 
par_vs_met_sev = merge(par_vs_met_sev, uncer[is.na(aft_1900), mean(range_sev_met, na.rm = T), .(var, reg)], by = c("var", "reg")) 
